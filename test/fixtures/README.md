...
| `floatcompare.jl` | `close_(a, b; atol = 1.0e-9)` — explicit-tolerance float compare, used by `test_dns.jl`, `test_pmv.jl`, `test_deltarho.jl` and `test_hydrophobicity.jl` wherever `check_float`'s fixed `DEFAULT_ATOL` is too tight |
