...
@testset "ParamTransform" begin
    ...
    #------------------------------------------------------------------
    #
    # Each of dns_prior/δρ_prior/c1_prior claims a support (LogNormal or
    # Normal) that θ's domain requirements (dns, δρ1, c1 > 0; δρ2, δρ3
    # unrestricted) are built around. These draw at volume (250_000 draws
    # each, 1_000_000 total) directly the priors.

    @testset "priors -> θ/ξ: dns_prior draws round-trip and never throw (250_000 draws)" begin
        Random.seed!(20_260_917)
        d_dns = dns_prior(7.0, 0.0, Solute[NonBiological(0.5, 0.01, "urea")])
        fixed = (1.05, -0.2, 0.1, 1.02)   # δρ1, δρ2, δρ3, c1 held fixed
        n = 250_000
        finite_ok = true
        roundtrip_ok = true
        for _ in 1:n
    ...
    @testset "priors -> θ/ξ: δρ_prior draws round-trip and never throws" begin
        ...
        δρ1_d, δρ2_d, δρ3_d = δρ_prior(0.0; σ_χ = 1.5)
    ...
    @testset "priors -> θ/ξ: all three priors combined, full ξ vector" begin
        ...
        d_dns = dns_prior(7.4, 0.05, Solute[NonBiological(0.15, 0.001, "sodium chloride")])
        δρ1_d, δρ2_d, δρ3_d = δρ_prior(2.0; σ_χ = 3.0)
        d_c1 = c1_prior(95.0)
        n = 250_000
        finite_ok = true
        roundtrip_ok = true
