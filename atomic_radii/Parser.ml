(* SPDX-License-Identifier: LGPL-2.1-or-later *)
(** Ion-string grammar: [elem (charge sign | sign charge | sign)]. *)

open Toks
open Printf
open Lexing
open Lexer

(** A parsed ion, e.g. ["fe3+"] -> [{element = "fe"; charge = 3}]. *)
type ion = { element : string; charge : int }

exception Parse_error of string

let parse_error (msg : string) : 'a = raise (Parse_error msg)

(** Combine a magnitude and sign into a signed charge.
    @param n magnitude digits, or [""] for a bare sign (implies 1)
    @param s ['+'] or ['-']
    @return the signed charge, e.g. [combine "3" '-' = -3]
    @raise Parse_error if [s] is neither ['+'] nor ['-'] *)
let combine (n : string) (s : char) : int =
    let magnitude = if n = "" then 1 else int_of_string n in
    match s with
    | '+' -> magnitude
    | '-' -> -magnitude
    | _ -> parse_error (sprintf "invalid sign char %c" s)

(** @param lexbuf position expected to be at end of input
    @raise Parse_error if the next token isn't [EOF] *)
let expect_eof (lexbuf : lexbuf) : unit =
    match token lexbuf with
    | EOF -> ()
    | _ -> parse_error "expected end of input"

(** Parse one ion.
    @param lexbuf ion-string content, positioned at the start
    @return the parsed element and signed charge
    @raise Parse_error on malformed input *)
let parse (lexbuf : lexbuf) : ion =
    match token lexbuf with
    | Elem e ->
        let charge =
            match token lexbuf with
            | Charge n -> (
                match token lexbuf with
                | Sign s -> expect_eof lexbuf; combine n s
                | _ -> parse_error "expected sign after charge")
            | Sign s -> (
                match token lexbuf with
                | Charge n -> expect_eof lexbuf; combine n s
                | EOF -> combine "" s (* bare sign; EOF already consumed *)
                | _ -> parse_error "expected charge or end of input after sign")
            | _ -> parse_error "expected charge or sign after element"
        in
        { element = e; charge }
    | Token_error c -> parse_error (sprintf "unrecognized character %c" c)
    | _ -> parse_error "expected element"

(** @param s ion string, e.g. ["fe3+"]
    @return the parsed element and signed charge
    @raise Parse_error on malformed input *)
let parse_string (s : string) : ion = parse (from_string s)

(** Inverse of {!parse}.
    @param ion element and signed charge
    @return the [ionic_radii] key format, e.g. [{element="fe";charge=3}] -> ["fe3+"]
    @raise Parse_error if [ion.charge = 0] (no db representation) *)
let to_string (ion : ion) : string =
    if ion.charge = 0 then
        parse_error "charge 0 has no ion string representation"
    else
        let sign = if ion.charge > 0 then '+' else '-' in
        sprintf "%s%d%c" ion.element (abs ion.charge) sign
