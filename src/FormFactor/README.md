...
Not every ion has both halves of the sum. `compute_form_factors` classifies each requested species into one of three tiers and logs anything short of a full resolution (`form_factor_log`):
...
Ions are deduplicated on build, preserving first-seen order, so a batch like `["fe3+", "fe3+", "o2-", "fe3+"]` produces one `fe3+` row in `t.tbl`, while `form_factors` still returns one output row per requested (possibly repeated) ion.
