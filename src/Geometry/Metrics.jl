...
`classify` and `blocked` are pure distance/containment tests against `crds`/
`rads`; they don't care where a query point `p` came from. That means they
work unchanged whether `p` is a `PlasticSequence.plastic_points(..., Val(3),
Val(:surface))` point (always `|p - centre| == radius`) or a `..., Val(3),
Val(:volume))` point (`|p - centre| < radius`, filling the interior) -- a
future volume estimator can reuse `classify`/`blocked` exactly as `SASA`
